...
- 强缓存：不会向服务器发送请求，直接从缓存中读取资源，在`chrome`控制台中可以看到请求返回200的状态码，并且`size`显示`from disk cache` 或 `from memory cache`
...
Symbol接触多吗
      ...
      对接各行业客户需求沟通，对产品整体业务的产出包括建立全套产品的指标体系、全量埋点方案、埋点数据校验、看板搭建、培训
使用和答疑。涉及的行业有：商业地产、金融、零售、汽车。负责的项目有 6 个；奔驰车载端、联联周边游 APP 端小程序端、上海
信托积分商城 H5、广州越秀地产 C 端 B 端、维信金科金融 APP 端、江苏征信 PC 端 APP 端； 2、 协助销售侧和项目经理侧完成售前方案数据分析部分材料，涉及项目有 4 个：拜耳农业小程序、奔驰车载端、兴业证券 APP 端、三 福小程序，其中通过 POC 的产出（指标体系、埋点和看板、洞察输出）为中标奔驰项目做出直接贡献； 3、 协助客户成功经理和项目经理完成日常客户对方舟产品使用和分析的问题； 4、 反馈方舟产品中包括 UBA 可视化看板、UP 画像和自研 SQL 查询平台的功能优化建议，迭代产品提升市场竞争力；
      ...
      灰度发布：
        ...
        2.资源新旧版本
        3.前端灰度发布的几种方式：
          3.1.服务端渲染应用-用户首先发起请求后，服务器并不会直接组装静态资源，而是先去灰度规则里获取名单，然后将灰度名单拿到进行判断处理来决定渲染那一套模板资源给到客户端，最终给处于灰度名单里的用户展示新版本页面，而非名单内的用户继续使用旧版本的页面内容
          通过规则判断返回哪个静态资源文件
          3.2.前后端半分离的应用
            打包后的资源和之前的资源在命名上会有区别，通过指定不同的规则加载不同的静态文件，当用户的请求被接收后，先通过getRule获取灰度规则来确定当前用户展示哪一个版本号的资源，然后返回带着资源版本的模板内容，客户端接收到该模板之后，再加载对应的版本号资源，从而达到灰度要求。
          3.3.客户端渲染的前端应用
            一般情况是前端代码打包后，布置到CDN上，然后利用nginx来请求到静态资源，这个时候不能通过后端通过一段获取灰度逻辑来控制版本，而此时前端数据请求都是异步ajax的方式
            解决思路1：可以在前端代码写2套内容，在页面渲染之前发起异步获取灰度规则的请求，将结果拿到后在客户端决定渲染哪一套页面
            缺点：前端代码体积变大，增加维护成本
            解决思路2：保持版本号来区分每次的迭代，只是在渲染前获取到正确的版本资源来渲染就可以解决
            灰度测试和灰度发布-经历和项目分开写(时间只针对经历)
            FP(first paint)
            FCP(first content paint)
    
            工作经历：
            XXX公司：
            1.中行项目前端负责人，参与需求调研、产品交付，顺利完成项目的一期交付
            2.项目首屏优化，通过性能监控工具，提升FP、FCP指标；
          
            3.从零到一完成知因风度产品，快速完成版本1.0 -> 3.3的迭代，全市场风险一键度量
            4.个人专利1件(同义词库)、团队专利2件(知识图谱中集成集团黑名单展示)
    
            车大大
            1.基于vue全家桶开发HMI车载娱乐系统(人工智能方向),语音智能控制驾驶,实现驾驶智能化
            2.参与技术选型Vue2.0(全家桶，门店管理平台前后端分离重构)
            
            3.解决了大屏项目订单信息不能及时同步的问题,websocket心跳保证长连接
            4.大屏图片进行压缩后加载，压缩率优化至30%
            5.websocket心跳，保证订单的同步问题
            
            恒泰软件
            erp转前端 angular1 + layuiERP管理系统，2017/2获得优秀实施工程师称号，2年的 ERP经验成功转到web前端开发
    
            项目1：风险数据智能化
              first painting 首次绘制
              first content painting 首次内容绘制
              1.优化了核心项目的首屏性能，通过性能监控工具，提升了FP、FCP指标
              2.知识库管理中，页面涉及大量图表，通过研究公司已有的BI系统，减少可视化图表的开发工作量，节省约50%开发时间
              3.自定义v-perssion指令，实现按钮级别的权限控制，
              4.基于zoomcharts的force布局，良好的force布局，让用户更好的体验
              基于黑灰名单查询名单的长列表优化，采用虚拟列表，解决长列表的性能问题，
              实现长页面的懒加载，预加载，提升页面性能，体验
    
            项目2：智慧营销|风度
              搭建项目脚手架，继承框架全家桶，项目CI/CD.提高开发-测试效率
              带领团队完成项目从零到一的建设
              熟悉工程化建设，推动团队基础建设，如 推动CI/CD的建设(jenkens+docker部署)，优化了构建流程
            项目3：北京银行知识图谱
              推动前后端分离，参与技术选型(vue2.x + resource -> axios)
              
            项目4：HMI车载系统
              封装25+组件
              自动驾驶上路实测，并实现各种语音功能
              遇到问题是毛刺信号导致canvas画面抖动，通过研究动画相关的库，最终选用TweenMax良好的补间动画效果，实现倒车画面的平滑展现。


自己封装一个vue-router，原理：Vue.use。插件是对象，执行插件中的install方法；如果插件是个函数，则执行这个函数。
在beforeCreated 之前使用，因为在created之后，$options已经初始化好了。需要把$router挂载到根组件

  超级简历开会员仿写
  自定义脚手架并且发布到npm
  scott说的120W的简历模板，截图对照修改
  https://blog.csdn.net/zdhui_fly/article/details/105136234
