  ...
  methods: {
    ...
    update(source, showtype, sourceTree) {
      ...
      nodeEnter
        ...
        .attr('stroke', d =>
          d.data.type === COMPANY || !d.depth ? '#FD7D00' : '#7A9EFF'
        )
        .attr('stroke-width', 1)
        .attr('rx', 5)
        .attr('ry', 5)
          ...
          setTimeout(() => {
            if (
              ...
              document
                ...
                .setAttribute('style', 'fill:#FFF1D7')
              document.querySelector(`text[type="${d.id}"]`).innerHTML = '+'
            } else {
              d.isOpen = true
      ...
      nodeEnter
        ...
        .text(d =>
          d.data.name.length > 9 ? d.data.name.substr(0, 9) : d.data.name
        )
